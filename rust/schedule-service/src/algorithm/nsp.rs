...
static mut SCHEDULE: Option<HashMap<Chromosomes, Schedule>> = None;
...
impl AsPhenotype for Chromosomes {
    fn as_schedule(&self) -> Schedule {     
        unsafe{
            match SCHEDULE {
                Some(ref schedule) => {
                    match schedule.get(self) {
                        Some(sh) =>{ 
                            return sh.get_copy()},
                        None => {
                            println!("sranje");
                        return  Vec::new() //Schedule{ schedule: Vec::new(), incomplete_schedule: false }
                        }
                    }
                },
                _ => {return Vec::new()}//Schedule{ schedule: Vec::new(), incomplete_schedule: false };}
            };
        };
    }
}
...
fn make_schedule(chromosomes: &Chromosomes) -> Schedule {
    let mut schedule: Schedule = Vec::new();//Schedule{ schedule: Vec::new(), incomplete_schedule: false };
    unsafe{
        match SIMULATOR {
            Some(ref sim) =>{
                let mut index: i32 = -1;
                ...
                for staff in  &sim.staff {
                    ...
                    for _ in 0..staff.num_of_shifts {
                        ...
                        let (day, shift, shift_exists) = get_day_and_shift(&get_available_shifts(&sim.shift_map, &assigned_shifts), chromosome.day_index);
    ...
    unsafe{
        match SCHEDULE {
            Some(ref mut schedule_map) => {
                schedule_map.insert(chromosomes.to_vec(),  schedule.get_copy());
            },
            None =>{ println!("none")}
        };
    }
...
fn get_day_and_shift(shift_map: &ShiftMap, day_idx: i8) -> (Day, Vertex, bool) {
    ...
    loop {
        ...
        if day.is_some(){    
            ...
            if vertex.is_some(){
                ...
                return (Day::Monday, Vertex{start: String::from("sh1"), end: String::from("sh2")}, false)
            }
        }
    }
